        ...
        [
            ...
            [
                ...
                'risposta' => [
                    ...
                    'punti' => [
                        [
                            'testo_principale' => "La società consociata Google che offre i servizi, tratta le tue informazioni ed è responsabile del rispetto delle leggi sulla privacy vigenti. Generalmente Google offre i propri servizi per i consumatori tramite una delle due società seguenti:",
                            'sottopunto_a' => "Google Ireland Limited, se gli utenti sono residenti nello Spazio economico europeo (paesi dell'Unione europea, oltre a Islanda, Liechtenstein e Norvegia) o in Svizzera.",
                            'sottopunto_b' => "Google LLC, con sede negli Stati Uniti, per il resto del mondo.",

                        ],
                        [
                            'testo_principale' => "La versione dei termini che regola il nostro rapporto, che può variare in base alle leggi locali.",
...
<body>
    ...
    <?php foreach($faqs as $faq) : ?>
        ...
        <?php foreach($faq['risposta'] as $key => $paragraph) : ?>
            <?php if(is_numeric($key)) :?>
                <p><?= $paragraph ?></p>
            <?php elseif($key == 'sottotitolo') :?>
                <h3><?= $paragraph ?></h3>
            <?php else: ?>

                <ol>
                    <?php foreach($faq['risposta']['punti'] as $punto) : ?>
                        <li>
                            <?=$punto['testo_principale'] ?>
                            <?php if(count($punto) > 1) : ?>
                                <ol>
                                    <li>
                                    <?= $punto['sottopunto_a'] ?>
                                    </li>
